-- Atom feed generator
-- Adapted from https://github.com/PataphysicalSociety/soupault-blueprints-blog/blob/main/plugins/atom.lua
...
if not Table.has_key(custom_options, "site_url") then
  Plugin.fail("custom_options.site_url is required for Atom feed generation")
end
...
if (size(entries) == 0) then
  Log.warning("No entries found for Atom feed, skipping generation")
  Plugin.exit()
end
